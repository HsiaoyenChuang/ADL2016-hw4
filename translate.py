      ...
      if current_step % FLAGS.steps_per_checkpoint == 0:
        ...
        for bucket_id in xrange(len(_buckets)):
          ...
          # If there is an EOS symbol in outputs, cut them at that point.
            ...
            sentence_res.append(" ".join([tf.compat.as_str(rev_fr_vocab[output]) for output in outputs]))
        print('sentence_res len',len(sentence_res),'sentence_res_input len',len(sentence_res_input))
        save_result(sentence_res_input, FLAGS.data_dir + 'valid_comp_result.txt', 'w')
        save_result(sentence_res,FLAGS.data_dir+'valid_result.txt','w')
        bleu = get_bleu(FLAGS.data_dir+'valid_result.txt',FLAGS.data_dir+'data/valid.es')
...
def test():
  with tf.Session() as sess:
    ...
    sentence_es_arr = []
    with open(FLAGS.data_dir+'data/valid.en','r') as infile:
        for sentence in infile:
          ...
          sentence_es_arr.append(" ".join([tf.compat.as_str(rev_fr_vocab[output]) for output in outputs]))
...
def decode():
  with tf.Session() as sess:
    ...
    # Decode from standard input
    sentence_es_arr = []
    with open(FLAGS.data_dir+'data/test.en','r') as infile:
        for sentence in infile:
          # Get token-ids for the input sentence.
          token_ids = data_utils.sentence_to_token_ids(tf.compat.as_bytes(sentence), en_vocab)
          # Which bucket does it belong to?
          ...
          # print(" ".join([tf.compat.as_str(rev_fr_vocab[output]) for output in outputs]))
          sentence_es_arr.append(" ".join([tf.compat.as_str(rev_fr_vocab[output]) for output in outputs]))
    save_result(sentence_es_arr,FLAGS.data_dir+'test_result.txt','w')
